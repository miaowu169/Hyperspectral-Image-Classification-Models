...
from utils import grouper, sliding_window, count_sliding_window,\
                  camel_to_snake
from HybirdSN import HybirdSN
from Involution import Involution2d
from transformer_conv import Wide_ResNet
from spectral_vit import Spect_Vit
...
def get_model(name, **kwargs):
    ...
    if name == 'nn':
    ...
    elif name == 'Involution':
        # patch_size = kwargs.setdefault('patch_size', 7)  # default = 11
        center_pixel = True
        model = Involution2d(n_bands, 192, kwargs.setdefault('patch_size', 11))
        print("model_parameters:{}".format(model.__repr__()))  # 打印参数
        # model = Cnn2D(n_bands, n_classes, patch_size=patch_size)
        lr = kwargs.setdefault('learning_rate', 0.001)  # 设置的0.001
        # optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        kwargs.setdefault('batch_size', 64)  # 30samples=128,10samples=64
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    elif name == 'transformer':
        # patch_size = kwargs.setdefault('patch_size', 7)  # default = 11
        patch_size = kwargs.setdefault('patch_size', 11)
        center_pixel = True
        model = Wide_ResNet(28, 10, 0.3, num_classes=n_classes, shape=patch_size)
        print("model_parameters:{}".format(model.__repr__()))  # 打印参数
        # model = Cnn2D(n_bands, n_classes, patch_size=patch_size)
        lr = kwargs.setdefault('learning_rate', 0.001)  # 设置的0.001
        # optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        kwargs.setdefault('batch_size', 128)  # 30samples=128,10samples=64
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    elif name == 'HybirdSN':
        patch_size = kwargs.setdefault('patch_size', 15)  # default = 21
        center_pixel = True
        self_attention = False
        model = HybirdSN(num_classes=n_classes,  self_attention=self_attention)
        lr = kwargs.setdefault('learning_rate', 0.001)
        # optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        kwargs.setdefault('batch_size', 64)  # ORI:256  10
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    elif name == 'Cnn2D':
        patch_size = kwargs.setdefault('patch_size', 21)  # default = 11
        center_pixel = True
        model = Cnn2D(n_bands, n_classes, patch_size=patch_size)
        lr = kwargs.setdefault('learning_rate', 0.001)  # 设置的0.001
        # optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
        ...
        kwargs.setdefault('batch_size', 64)  # 30samples=128,10samples=64
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    elif name == 'hamida':  # 注意patch为21时，效果奇差，建议设置为默认
        patch_size = kwargs.setdefault('patch_size', 5)  # default 5
        kwargs.setdefault('epoch', 300)
        center_pixel = True
        model = HamidaEtAl(n_bands, n_classes, patch_size=patch_size)
        lr = kwargs.setdefault('learning_rate', 0.001)  # default 0.01
        # optimizer = optim.SGD(model.parameters(), lr=lr, weight_decay=0.0005)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        kwargs.setdefault('batch_size', 100)  # 100,128,256  default:100
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    elif name == 'lee':
        ...
        kwargs.setdefault('batch_size', 100)  # default 60
        epoch = kwargs.setdefault('epoch', 50)  # default 30
        lr = kwargs.setdefault('lr', 0.001)  # default 0.05
        center_pixel = True
        # We assume patch_size = 64
        kwargs.setdefault('patch_size', 11)  # default=64
        ...
        criterion = (nn.CrossEntropyLoss(), lambda rec, data: F.mse_loss(rec, data[:,:,:,patch_size//2,patch_size//2].squeeze()))
    elif name == 'boulch':
        kwargs['supervision'] = 'semi'
        kwargs.setdefault('patch_size', 1)
        kwargs.setdefault('epoch', 100)
    ...
    model = model.to(device)
    epoch = kwargs.setdefault('epoch', 300)  # 100
...
class Cnn2D(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, patch_size=21):
        ...
        kernel_size = 3
        nb_filter = 16
        self.vit = Spect_Vit(self.input_channels, 196, patch_size, 1024)  # dim:1024
        self.conv1 = nn.Sequential(nn.Conv2d(self.input_channels, 4*nb_filter, kernel_size=1, stride=1, padding=0),
                                   nn.BatchNorm2d(nb_filter*4),
                                   nn.LeakyReLU(),
                                   )  # 改变了通道数
        self.maxpool = nn.MaxPool2d(2, 2, 1)
        self.conv2 = nn.Sequential(
            nn.Conv2d(4*nb_filter, 8*nb_filter, kernel_size, padding=1),
            nn.BatchNorm2d(8*nb_filter),
            nn.LeakyReLU(),
        )  # 通道数增加
        self.conv3 = nn.Sequential(
            nn.Conv2d(8*nb_filter, 16*nb_filter, kernel_size, padding=1),
            nn.BatchNorm2d(16*nb_filter),
            nn.LeakyReLU(),
        )
        ...
        self.fc_2 = nn.Sequential(
            # nn.Linear(2048, 512),  # ori 1024,after 2048
            # nn.BatchNorm1d(512),
            # nn.ReLU(),
    ...
    def forward(self, x):
        ...
        x = self.fc_1(x)
        # out1 = torch.cat((x, out), dim=1)  # 级联
        # out1 = torch.add(x, out)  # 相加
        x = self.fc_2(x)
        return x
        # out1 = self.fc_2(out1)
        # return out1
...
class HuEtAl(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, kernel_size=None, pool_size=None):
        ...
        if kernel_size is None:
           ...
           kernel_size = math.ceil(input_channels / 9)  # 向上取整
    ...
    def forward(self, x):
        ...
        # print(x.size())
        x = x.squeeze(dim=-1).squeeze(dim=-1)  # the dim of x is 2,(shape:100*204) ,so x.sequeeze is useless now.
        x = x.unsqueeze(1)  # add a dim ,the shape is 100*1*204
        # print(x.shape)  # 100*1*204
        x = self.conv(x)  # input
...
class SharmaEtAl(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, patch_size=64):  # default =64
...
class LiuEtAl(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.squeeze()
        # print(x.shape)
        x_conv1 = self.conv1_bn(self.conv1(x)).contiguous()
        # x_conv1 = self.conv1_bn(self.conv1(x))
        x = x_conv1
        x_pool1 = self.pool1(x).contiguous()
...
def train(net, optimizer, criterion, data_loader, epoch, scheduler=None,
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the network"):
        ...
        for batch_idx, (data, target) in tqdm(enumerate(data_loader), total=len(data_loader)):  # 调用内置函数
            # Load the data into the GPU if required
            # print(data.shape), print(target.shape)
            data, target = data.to(device), target.to(device)
            # print(data.shape)  # 100*204
            optimizer.zero_grad()
            if supervision == 'full':
                output = net(data)
                # target = target - 1
