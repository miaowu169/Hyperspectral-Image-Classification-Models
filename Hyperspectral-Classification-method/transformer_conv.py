...
class AugmentedConv(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(torch.cat((conv_out, attn_out), dim=1).shape)  # ??????2*500*11*11????
...
# augmented_conv1 = AugmentedConv(in_channels=3, out_channels=20, kernel_size=3, dk=40, dv=4, Nh=4, relative=True, /
# padding=1, stride=2, shape=16).to(device)
...
class Wide_ResNet(nn.Module):
    def __init__(self, depth, widen_factor, dropout_rate, num_classes, shape):
        ...
        n_Stages = [20, 20* k, 40 * k, 60 * k]
        self.conv1 = AugmentedConv(in_channels=204, out_channels=n_Stages[0], kernel_size=3, dk=dk_k * n_Stages[0],
                                   dv=int(dv_v * n_Stages[0]), shape=shape, Nh=Nh, relative=True)  # 更改input
