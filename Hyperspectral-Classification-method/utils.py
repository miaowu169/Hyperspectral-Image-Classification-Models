...
def show_results(results, vis, label_values=None, agregated=False):
    ...
    if agregated:
        ...
        Class_accuracy = [r["Class accuracy"] for r in results]
        ...
        Class_accuracy = results["Class accuracy"]
    ...
    if agregated:
        ...
        for label, score, std in zip(label_values, Class_accuracy_mean,
                                 Class_accuracy_std):
                text += "\t{}: {:.04f} +- {:.04f}\n".format(label, score, std)
    else:
        #for label, score in zip(label_values, F1scores):
        for label, score in zip(label_values[1:], Class_accuracy):
            L.append(label)
            S.append(round(score, 4))
            text += "\t{}: {:.03f}\n".format(label, score)
    text += "---\n"

    # for i in ["Overall_Accuracy", "Average_Accuracy", "Kappa"]:  # TODO --***********--
    #     L.append(i)
    # for j in [accuracy, Average_accuracy,  kappa]:
    #     S.append(round(j, 4))
    # work = pd.ExcelWriter(r".\results\2.xlsx")  # TODO --***********--
    # dataframe = pd.DataFrame(S, index=L)  # TODO 保留四位小数,写入Excel
    # dataframe.to_excel(work, sheet_name='1')
    # TODO 实现数据的追加
    # df1 = pd.DataFrame(pd.read_excel(r"C:\Users\h'p\Desktop\1.xlsx", sheet_name='1'))  # 读取原数据文件和表
    # writer = pd.ExcelWriter(r"C:\Users\h'p\Desktop\1.xlsx", engine='openpyxl')
    # book = load_workbook(r"C:\Users\h'p\Desktop\1.xlsx")
    # writer.book = book
    # writer.sheets = dict((ws.title, ws) for ws in book.worksheets)
    # df_rows = df1.shape[0]  # 获取原数据的行数
    # dataframe.to_excel(writer, sheet_name='1', startrow=df_rows + 1, index=False, header=False)
    #work.save()  # TODO --***********--
