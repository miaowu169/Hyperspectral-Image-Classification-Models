...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG, patch_size=21):
    ...
    if dataset_name == 'PaviaC':
        ...
        ** Links for houston dataset are not working as on April 27, 2021.
        ...
        and unzip them.
        """
        # img = open_file(folder + 'houston_ori.mat')
        # img = img['houston']
        img = open_file(folder + 'Houston.mat')
        # print(img.keys())
        img = img['Houston']  # 349 1905 144
        # print(img.shape)
        # rgb_bands = (43, 21, 11)
        rgb_bands = (43, 60, 21)
        gt = open_file(folder + 'Houston_gt.mat')['Houston_gt']  # 不是tr是te
        # gt = open_file(folder + 'houston_gt.mat')['houston_gt']
        # print(gt.keys())  # 349 1905
        ignored_labels = [0]
    ...
    # data = preprocessing.scale(data)hh
...
class HyperX(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        x, y = self.indices[i]
        x1, y1 = x - self.patch_size // 2, y - self.patch_size // 2  # 因为在此处patch_size为1，所以为0 x1,x2没有变化
        ...
        # Remove unused dimensions when we work with invidual spectrums
            ...
            data = data[:, 0, 0]  # 1 ndim
            label = label[0, 0]

        # Add a fourth dimension for 3D CNN
        if self.patch_size > 1:
            # Make 4D data ((Batch x) Planes x Channels x Width x Height)
            data = data.unsqueeze(0)  # 在此处添加一个维度是为了
        # print("shape{},label{}".format(data.size(), label))
        # print(data, data.dim())  # 一维的数据
        return data, label
