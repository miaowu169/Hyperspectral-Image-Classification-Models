...
parser.add_argument('--dataset', type=str, default='IndianPines', choices=dataset_names,
                    help="Dataset to use: IndianPines; PaviaU; Salinas;Houston")
parser.add_argument('--model', type=str, default='liu',
...
group_dataset.add_argument('--training_sample', type=float, default=5,  # TODO 更改样本的数量
                    help="Percentage of samples to use for training (default: 10%)")
group_dataset.add_argument('--sampling_mode', type=str, help="Sampling mode"
                    " (random sampling or disjoint, default: random)",  # TO-`DO 取样的模式
                    default='fixed')  # fixed//random
...
group_train.add_argument('--epoch', type=int, default=800, help="Training epochs (optional, if"
                    " absent will be set by the model)")
group_train.add_argument('--patch_size', type=int, default=15, help="Size of the spatial neighbourhood (optional, if "
                                                                    "absent will be set by the model)")
...
if palette is None:
    ...
    palette = {0: (0, 0, 0)}
    for k, color in enumerate(sns.color_palette("huls", len(LABEL_VALUES) - 1)):
...
for run in range(N_RUNS):
    if TRAIN_GT is not None and TEST_GT is not None:
        ...
        PATCH_SIZE1 = 15  # TODO 若要更改块大小的话，此处也要一并修改！******************！
        ...
        for x, y in indices:
            if mask[x + PATCH_SIZE1//2, y + PATCH_SIZE1 // 2] is not 0:  # gt位置矫正
                mask[x + PATCH_SIZE1 // 2, y + PATCH_SIZE1 // 2] = gt[x + PATCH_SIZE1 // 2, y + PATCH_SIZE1 // 2]
        train_gt = mask
        # -------------------------------------------------------------------------------
        # train_gt = get_train(train_gt, 5, N_CLASSES)  # 选完了验证集样本后，再做一个样本挑选
    print("{} training samples & {} testing samples selected (over {})".format(np.count_nonzero(train_gt),
                                                np.count_nonzero(test_gt), np.count_nonzero(gt_)))
    ...
    if MODEL == 'SVM_grid':
        ...
        prediction = prediction.reshape(img.shape[:2])  # 369,1925
        prediction = prediction[(PATCH_SIZE // 2):-(PATCH_SIZE // 2),
                     (PATCH_SIZE // 2):-(PATCH_SIZE // 2)]  # 多加的
        sio.savemat('./pic/Augsburg-SGD.mat',{'SGD':prediction})
        # print(prediction.shape)
    elif MODEL == 'SVM':
        ...
        sio.savemat('./pic/Augsburg-SVM.mat',{'SVM':prediction})
        ...
        try:
            ...
            pass
        # print(img.shape)  # 三维
        probabilities = test(model, img, hyperparams)  # .squeeze(axis=0)
        prediction = np.argmax(probabilities, axis=-1)
        # print(prediction.shape)
        # /****************/
        prediction = prediction[(PATCH_SIZE // 2):-(PATCH_SIZE // 2), (PATCH_SIZE // 2):-(PATCH_SIZE // 2)]  # 349*1905
        gt = gt[(PATCH_SIZE // 2):-(PATCH_SIZE // 2), (PATCH_SIZE // 2):-(PATCH_SIZE // 2)]
        # 这两句句代码必须加，做了填充的话

    run_results = metrics(prediction, test_gt, ignored_labels=hyperparams['ignored_labels'], n_classes=N_CLASSES)
    # print(gt.shape)
    #gt_f = gt[(PATCH_SIZE // 2):-(PATCH_SIZE // 2), (PATCH_SIZE // 2):-(PATCH_SIZE // 2)]
    # TODO 用了svm或者rbf，上面这句代码要加，运行其他的代码就将这句代码注释掉
    mask = np.zeros(gt_.shape, dtype='bool')
    # print(mask.shape)
    # print(prediction.shape)
